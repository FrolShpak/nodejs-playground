...
function routes() {
    ...
    googleTechGuideRouter
        .route('/challenges/sumNumbers')
        .get((req, res)=> {
            res.status(200);
            return res.send(`Given a string, return the sum of the numbers appearing in the string, ignoring all other characters. A number is a series of 1 or more digit chars in a row. (Note: Character.isDigit(char) tests if a char is one of the chars '0', '1', .. '9'. Integer.parseInt(string) converts a string to an int.)
            ...
            Example payload for post:
            {
                "inputString": "abc123xyz"
            }`);
        })
        .post(controller.sumNumbers);
